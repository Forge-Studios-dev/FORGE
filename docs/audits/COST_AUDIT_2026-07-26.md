...
| 6 | AWS S3 | ~~`scripts/setup-aws-forge.sh` (last IaC touchpoint for the media bucket) sets public-access-block, CORS, and a CloudFront OAC + least-privilege IAM policy — all good. **No S3 lifecycle rule is configured anywhere in the script or repo**~~ **Fixed 2026-07-26**: added `put-bucket-lifecycle-configuration` step (aborts incomplete multipart uploads after 7 days, matching the BullMQ `removeOnFail: 7 * 24 * 3600` pattern used elsewhere). Script-only change — applies next time `setup-aws-forge.sh` runs against the bucket (idempotent, additive, no effect on existing completed objects). | Was unknown without AWS console access — cold/orphaned upload objects (abandoned multipart sessions, per `MUX_COST_OPS.md`'s "Orphan S3" checklist item) had no automatic expiration. | Done: incomplete-multipart-upload expiry automated. **Not done**: transition/expire orphaned `uploads/` temp-prefix *completed* objects — still needs AWS console access to see actual object count/age before picking a safe expiration window; left as `MUX_COST_OPS.md` manual check #5 until then. | Removes the open-ended growth from abandoned multipart sessions; exact $ unknown without console access to current object inventory. | Low (down from Medium) — the automatic-abort gap is closed; remaining risk is the smaller "completed but orphaned" object case, unchanged from before |
...
- **#3** — ~~Raise Neon pool idle timeout (30s → ~90–120s) to cut reconnect churn.~~ **Done** — shipped in #157 (`fix(api): raise Neon pool idle timeout to reduce connection churn`).
- **#6** — ~~Add S3 lifecycle rule for incomplete multipart uploads~~ **Done** — `put-bucket-lifecycle-configuration` step added to `scripts/setup-aws-forge.sh` (2026-07-26); applies next time the script runs against the bucket. The orphaned-*completed*-object sub-item still needs AWS console access to size a safe window first.
...
- **Finding #3 (Neon idle timeout)** — shipped in #157. Rollback is reverting `DB_POOL_IDLE_TIMEOUT_MS` (or the code default) back to 30s; single env var / config change, no migration, no data risk.
...
- **Finding #6 (S3 lifecycle rule)** — shipped 2026-07-26 in `scripts/setup-aws-forge.sh`. Rollback is `aws s3api delete-bucket-lifecycle --bucket <bucket>` or removing the `put-bucket-lifecycle-configuration` step from the script; no data risk (rule only affects *incomplete* multipart sessions, never completed objects).

**Implemented from this report: finding #1** (shipped as part of the production incident response, `fly scale count 2`, merged via the `#153`/`#154` PR chain), **finding #3** (`DB_POOL_IDLE_TIMEOUT_MS`, PR #157), and **finding #6** (S3 lifecycle rule, `scripts/setup-aws-forge.sh`, 2026-07-26) — all config-only, low-risk, no migration, matching the quick-win bar in `forge-production-stability.md`'s pre-deployment gate. Finding #2 (bluegreen) remains a structural proposal, not urgent, awaiting go-ahead. Findings #7/#8 need credentials not available in any session so far.
