...
|---|---|---|
| Shorts ranking uses freshness+views+likes (`shorts-rank.util.ts`), not completion/rewatch like YouTube's actual model | Checked 2026-08-13: `avgWatchPercent` (the closest existing signal) is only computed on-demand over `WatchHistory` for a creator+time-window (Studio dashboard) — there is no per-video cached field. Wiring it into the hot Shorts-feed path as-is means either an expensive live aggregate per candidate on every request, or a new precompute pipeline (cron job, storage field, staleness tolerance). Both are real engineering decisions, not a signal-swap. | Eng lead — decide precompute cadence/storage before building |
