...
| Taxonomy naming/shape is skill-economy-flavored | Medium | `SkillTag` entity, `explore/skills/*` routes, skill-overlap mentorship matching reusing the same taxonomy tables | YouTube's category/topic model is flat and cosmetic, not a skills hierarchy | Depends on the same product decision; if resolved toward YouTube parity, plan a rename/reshape of `SkillTag`→ generic `Tag`/topic and drop the hierarchy's coupling to mentorship |
