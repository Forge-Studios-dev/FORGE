...
export function CreatorCoursesPanel({ creatorId, username }: Props) {
  const { coursesEnabled, skillEconomyLms } = useSkillFeatures();
  ...
  if (!coursesEnabled) return null;
  if (coursesLoading || (skillEconomyLms && programsLoading)) return null;
