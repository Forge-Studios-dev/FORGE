    ...
    <main className="mx-auto max-w-3xl px-5 py-8 md:px-12">
      ...
      <Tabs
        ...
        tabs={[
          ...
          ...(skillEconomyLms ? [{ id: 'programs', label: 'Programs' }] : []),
        ]}
      />
...
function ProgramsTab({ userId, username }: { userId?: string; username?: string }) {
  const qc = useQueryClient();
  ...
  const updateProgramMutation = useMutation({
    mutationFn: async ({
      ...
      programId: string;
      patch: { isPublished?: boolean; priceCents?: number };
    }) => {
      await api.patch(`/creators/me/programs/${programId}`, patch);
    },
    onSuccess: () => void qc.invalidateQueries({ queryKey: ['my-programs', userId] }),
  });
    ...
    <>
      <section className="glass-panel mb-8 space-y-3 rounded-xl p-6">
        ...
        <Input
          ...
          placeholder="Price in cents (0 = free, min 100 for paid checkout)"
        />
        <textarea
          value={description}
          onChange={(e) => setDescription(e.target.value)}
            ...
            {(programs ?? []).map((program) => (
              <li key={program.id} className="glass-panel rounded-xl p-4 text-sm">
                ...
                <div className="mt-3 flex flex-wrap gap-2">
                  <Button
                    ...
                    onClick={() =>
                      updateProgramMutation.mutate({
                        ...
                        patch: { isPublished: !program.isPublished },
                      })
                    }
                  >
                  ...
                  {username ? (
                    <Link href={`/${username}/programs/${program.slug}`} className="text-xs text-primary">
                      View public page
                    </Link>
                  ) : null}
                </div>
