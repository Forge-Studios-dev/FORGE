...
export default function StudioCourseDetailPage() {
  return (
    <SkillFeatureGate feature="courses">
      <StudioCourseDetailPageInner />
    </SkillFeatureGate>
  );
}

function StudioCourseDetailPageInner() {
  ...
  const { data: cohorts } = useQuery({
    ...
    enabled: !!courseId && !!user?.id && isCreator && skillEconomyLms,
    queryFn: async () => {
  ...
  const { data: attachableVideos = [] } = useQuery({
    queryKey: ['studio-course-videos', user?.id],
    enabled: !!user?.id && isCreator,
    queryFn: async () => {
      const { items } = await fetchStudioLibrary({ status: 'ready', limit: 100 });
      return items;
    },
  });
  ...
  const createLessonMutation = useMutation({
    mutationFn: async () => {
      await api.post(`/creators/me/courses/${courseId}/lessons`, {
        ...
        lessonType: lessonVideoId ? 'video' : 'text',
        videoId: lessonVideoId || undefined,
      });
    },
    onSuccess: () => {
      setLessonTitle('');
    ...
    <main className="mx-auto max-w-3xl px-5 py-8 md:px-12">
      ...
      <section className="glass-panel mb-8 space-y-3 rounded-xl p-6">
        ...
        <textarea
          ...
          placeholder="Lesson notes (optional when using a video)"
        ...
        <label className="block text-xs text-on-surface-variant">
          ...
          >
            <option value="">Text-only lesson</option>
                ...
                <div className="flex items-start justify-between gap-3">
                  <p className="font-medium">
                    ...
                    {lesson.lessonType === 'video' ? (
                      <span className="ml-2 text-xs font-normal text-primary">Video</span>
                    ) : null}
                  </p>
